...
from marimba.core.schemas.header import MetadataHeader, HeaderMergeConflictError
...
def test_valid_merge() -> None:
    first_header = MetadataHeader(TestHeader(a=None, b=0))
    second_header = MetadataHeader(TestHeader(a="test", b=0))

    result = reduce(MetadataHeader.__add__, [first_header, second_header])
...
def test_invalid_merge() -> None:
    first_header = MetadataHeader(TestHeader(a="other", b=0))
    second_header = MetadataHeader(TestHeader(a="test", b=0))
...
def test_ifdo_Header() -> None:
    first_header = MetadataHeader(ImageSetHeader(image_set_name="", image_set_uuid="", image_set_handle=""))
    first_header.header.image_context = ImageContext(name="Test")
    second_header = MetadataHeader(ImageSetHeader(image_set_name="", image_set_uuid="", image_set_handle=""))
