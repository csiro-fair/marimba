...
def check_input_args(source_path: str, file_extension:str , exiftool_path:str):
...
def catalogue_files(
    ...
    check_input_args(source_path, file_extension,exiftool_path)
    dirstoprocess =[]
    with Progress(SpinnerColumn(),TextColumn("[progress.description]{task.description}"),
        ...
        for item in glob.glob(os.path.join(source_path,glob_path,'.'),recursive=True):
            source = os.path.join(source_path,os.path.dirname(item),f'*.{file_extension}')
            if glob.glob(source):
